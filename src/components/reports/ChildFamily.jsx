  ...
  const downloadExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(
      filteredData.map((item, index) => ({
        S_no: toggle.showSerialNumber ? index + 1 : undefined,
        Check_In_Code: toggle.showCheckInCode ? item.checkInCode : undefined,
        Family_Name: toggle.showFamilyName ? item.familyName : undefined,
        Child_Name: toggle.showChildName
          ? `${item.firstName} ${item.lastName} (${item.nickName})`
          : undefined,
        Gender: toggle.showGender ? item.gender : undefined,
        Status: toggle.showStatus ? item.status : undefined,
        Admission_Date: toggle.showAdmissionDate
          ? item.admissionDate
          : undefined,
        Schedule: toggle.showSchedule ? item.schedule : undefined,
        Parent_Note: toggle.showParentNote ? item.parentNotes : undefined,
      }))
    );
